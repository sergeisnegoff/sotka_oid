...
@section('content')
    <div class="page-content browse container-fluid">
        ...
        <div class="row">
            <div class="col-md-12">
                <form class="panel panel-primary" method="post" action="{{ route('voyager.salesystems.updateTable') }}">
                    @csrf
                    <div class="panel-body">
                        <table class="table table-bordered">
                            ...
                            <tr>
                                <td  width="160px">Скидки от объёма</td>
                            ...
                            @foreach ($priceRange as $range)
                                <tr>
                                    <td>{{ $range }}</td>
                                    @foreach ($categories as $category)
                                        <td><input type="number" name="priceRange[{{ $range }}][{{ $category['id'] }}]" value="{{ @$values[$range][$category['id']] }}" class="form-control"><input
        ...
        $(function () {
            $('[name*=category]').on('change', function () {
                if (!$(this)[0].hasAttribute('data-parentid')) {
                    if ($(this).is(':checked'))
                        $('[data-parentid='+$(this).data('id')+']').attr('checked', true);
                    else
                        $('[data-parentid='+$(this).data('id')+']').attr('checked', false);
                }
            })
        })
    </script>
