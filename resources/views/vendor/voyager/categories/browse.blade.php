...
@section('css')
    ...
    <link href="{{ asset('/js/treetable/css/jquery.treetable.css') }}"></link>
    <link href="{{ asset('/js/treetable/css/jquery.treetable.theme.default.css') }}"></link>
@stop
...
@section('javascript')
    ...
    <script>
        ...
        $('body').on('click', 'tr .delete', function (e) {
    ...
    <script src="{{ asset('/js/libs/treetable/jquery.treetable.js') }}"></script>
    <script>
        $(function () {
            cloneTableToTree();
            ...
            $('#treetable tr[data-tt-parent-id=0]').each(function () {
                if ($(this).siblings('[data-tt-parent-id=' + $(this).data('tt-id') + ']').length) {
                    $('<a></a>', {
                        'href': '#',
                        'data-id': $(this).data('tt-id'),
                        'class': 'glyphicon glyphicon-arrow-right'
                    }).appendTo($(this).find('td').first());

                    $('[data-tt-parent-id="' + $(this).data('tt-id') + '"]').hide();
                }
            })

            $('#treetable tr[data-tt-parent-id=0] td:first-child > a[data-id]').on('click', function (e) {
                e.preventDefault();
                $(this).toggleClass('glyphicon-arrow-right glyphicon-arrow-down')
                let id = $(this).data('id');
                $(this).closest('tr').siblings('[data-tt-parent-id=' + id + ']').slideToggle();
            })
        });

        function cloneTableToTree(parent_id = 0) {
            $('#raw-tree').find('tr[data-tt-parent-id=' + parent_id + ']').each(function () {
                let clone = $(this).clone();
                if (parent_id == 0)
                    clone.find('td:nth-child(2)').css('font-weight', 'bold')
                else
                    clone.find('td:nth-child(2)').css('font-style', 'italic')

                clone.appendTo('#treetable');
                cloneTableToTree($(this).data('tt-id'));
            })
        }
    </script>
@stop
