                ...
                <div class="col-12">
                    <div class="box__ptofile-currentorder">
                        ...
                        @foreach ($cart as $order)
                            @if(!isset($order['id'])) @continue @endif
                           {{-- @php
                                $amount = 0;
                                $changes = false;
                            ...
                            @endphp--}}
                            <form method="POST" id="order-form" class="box__item" data-order-id="{{$order['id']}}" action="{{ route('preorder_create') }}">
                                @csrf
    ...
    <script>
        ...
        $(function () {
            ...
            $('body').on('change', '.cart_preorder_product', function () {
                ...
                $.post("{{ route('cart.updatePreOrderQty') }}", {id: product_id, qty: $(this).val()}, function (result) {
                    $.get("{{ route('preorder_cart_ajax') }}", function (result) {
